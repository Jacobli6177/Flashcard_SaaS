...
export default function Home() {
  ...
  const handleSubmit = async () => {
    try {
      const checkoutSession = await fetch('/api/checkout_session', {
        method: 'POST',
        headers: {
          origin: 'http://localhost:3000', // Ensure the correct origin header
        }
      });
      ...
      const { error } = await stripe.redirectToCheckout({
        sessionId: checkoutSessionJson.id, // Correct spelling of sessionId
      });
  ...
  const handleSubmitBasic = async () => {
    try {
      const checkoutSession = await fetch('/api/checkout_sessionBasic', {
        method: 'POST',
        headers: {
          origin: 'http://localhost:3000', // Ensure the correct origin header
        }
